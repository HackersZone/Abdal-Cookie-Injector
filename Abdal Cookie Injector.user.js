// ==UserScript==
// @name           Abdal Cookie Injector
// @namespace      HackersZone
// @description    Inject Cookie String From Wireshark Dump Into Any Webpage
// @version 1.0
// @include        *
// @exclude	   https?://hackers.zone/*
// ==/UserScript==
...
(function (){
	...
	*/
	cI.createDiv = function(){
		...
		cI.dialog.innerHTML = "<div align='center'><b>Abdal Cookie Injector</b><br/><input type='text' id='cookieInjectorCookie'/><br/><button onclick='cookieInjector.writeCookie();'>OK</button><button onclick='cookieInjector.hide();'>Cancel</button></div>";
		cI.dialog.style.display = "none";
		cI.dialog.style.position = "fixed";
		cI.dialog.style.opacity = "0.8";
		cI.dialog.style.top = "40%";
		cI.dialog.style.background= "#ffbe10";
	...
	*/
	cI.writeCookie = function(){
		...
		alert("All Cookies Have Been Written , Flame Turn On ;)");
		cI.hide();
	}
	...
	*/
	...
	/**
	* Handle all keypresses, we are looking for an ALT-a key-combo. Since we can't detect
	...
	*/
	cI.keyPress = function (e){
		//Check to see if "a" is pressed after ALT
		if(e.keyCode == 65 && cI.ctrlFire){
			if(!cI.visible){
				cI.show();
			}else{
				cI.hide();
